...
\paragraph{State Debouncing} The system tracks previous button state (\texttt{\_previousAButtonState}) to detect rising edge transitions, preventing accidental double-triggers from single button presses. This ensures one recording session per button press/release cycle.
...
\paragraph{Dynamic Positioning} The \texttt{LateUpdate()} method (lines 154--180) repositions the canvas 1 meter in front of the camera each frame:
