...
\subsubsection*{Tool Selection Prompt} Instructs GPT-4.1 to analyze user natural language input and return structured JSON identifying which tool to invoke. Stored in \texttt{PromptSettings.toolSelectionPrompt} (Assets/Scripts/Core/Config/PromptSettings.cs, lines 16--107), the prompt explicitly defines eight available tools:
...
\subsubsection*{Example Tool Selection Prompt Fragment}

The following excerpt from \texttt{toolSelectionPrompt} illustrates the instruction format for tool \texttt{create\_circular\_orbit}:
...
OUTPUT FORMAT (strict JSON only)
{
  ...
  "tool": "create_circular_orbit" | ... | null,
  "parameters": {
...
This structured format enables GPT-4.1 to reliably extract user intent and generate valid tool invocation JSON. The prompt includes routing guidance to distinguish between simulation control requests (handled by Hub tools) and educational queries (routed to Mission Space specialists).

\subsubsection*{Response Prompt} Generates natural language explanations of tool execution results. Stored in \texttt{PromptSettings.responsePrompt} (Assets/Scripts/Core/Config/PromptSettings.cs, lines 111--197), it includes explicit disambiguation guidance to prevent confusion between:
\begin{itemize}
...
Mission Space specialists (ISS, Hubble, Voyager) use the \texttt{specialistSystemPrompt} (Assets/Scripts/Core/Config/PromptSettings.cs, lines 229--267), which frames the agent as an enthusiastic mission expert focused on education rather than simulation control. Character configuration occurs through \texttt{MissionConfig} ScriptableObject assets:
...
The eight tools are defined in \texttt{ToolSchemas.json} (Assets/Resources/Tools/ToolSchemas.json, 168 lines) with complete JSON Schema specifications. Table~\ref{tab:tool_constraints} documents parameter constraints enforced by the \texttt{ToolRegistry} validation system.
...
The \texttt{OpenAIClient} class (Assets/Scripts/AI/Services/OpenAIClient.cs, 151 lines) implements asynchronous HTTP communication with OpenAI's Responses API endpoint (\texttt{/responses}).
