...
This appendix provides detailed technical implementation specifications for the conversational agent system described in Section~\ref{subsec:agent_implementation}. All class names, method signatures, file paths, and code excerpts are verified against the Unity project source code.
...
\begin{tabular}{lrl}
\hline
\textbf{Prompt Component} & \textbf{Size} & \textbf{Purpose} \\
\hline
\texttt{toolSelectionPrompt} & 460 lines & Interprets user intent, returns tool JSON \\
\texttt{responsePrompt} & 270 lines & Generates natural language responses \\
\texttt{specialistSystemPrompt} & 412 lines & Frames mission specialist character \\
...
\paragraph{Tool Selection Prompt (460 lines)} Instructs GPT-4.1 to analyze user natural language input and return structured JSON identifying which tool to invoke. The prompt explicitly defines eight available tools:
...
\paragraph{Response Prompt (270 lines)} Generates natural language explanations of tool execution results. Includes explicit disambiguation guidance to prevent confusion between:
\begin{itemize}
...
\paragraph{Non-Tool Response Template} Handles conversational interactions that do not require tool execution, such as greetings (``Hello, I'm Mission Control''), capability inquiries (``What can you do?''), and educational questions.
